...
const formatNumber = (val, decimals = 2, fallback = '0.00') => {
...
export default function TradingDashboard() {
  ...
  const maxBatt = Number(selectedNode.maxBattery || selectedNode.maxBatteryKWh || 0);
  const currBatt = Number(selectedNode.battery || selectedNode.batteryKWh || 0);
  const hasBattery = selectedNode.hasBattery !== false && maxBatt > 0;
  const batteryPercent = hasBattery && maxBatt > 0
    ? Math.min(100, Math.round((currBatt / maxBatt) * 100))
    : 0;

  const epsilon = typeof selectedNode.epsilon === 'number' ? selectedNode.epsilon : (Number(selectedNode.epsilon) || 0.25);
      ...
      <div>
        <div className="p-4 border-b border-slate-800/80 flex items-center justify-between bg-slate-900/60 sticky top-0 backdrop-blur-md z-10">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <h2 className="text-sm font-bold text-slate-100 leading-none">{selectedNode.name || selectedNode.id}</h2>
        ...
        <div className="p-4 space-y-4">
          ...
          {!isGrid && (
            <div className="glass-card p-3 rounded-xl border border-slate-800 space-y-2 bg-gradient-to-br from-slate-900/90 to-slate-950/90">
              ...
              {selectedNode.stateKey && typeof selectedNode.stateKey === 'string' && (
                <div className="pt-1">
                  <div className="text-[9px] text-slate-500 uppercase font-mono mb-1">State Vector (S - 6 Dimensions):</div>
                  <div className="flex flex-wrap gap-1 font-mono text-[9px]">
                    {selectedNode.stateKey.split('|').map((part, i) => {
                      const isCycle = ['MORNING_RAMP', 'SOLAR_GLUT', 'EVENING_PEAK', 'OFF_PEAK_NIGHT'].includes(part);
                      return (
                        <span
                          key={i}
          ...
          {!isGrid && (
            <div className="glass-card p-3.5 rounded-xl border border-slate-800 space-y-3">
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between items-center text-xs">
                  ...
                  <span className="font-mono text-cyan-300 text-[11px] font-bold">
                    ε = {formatNumber(epsilon, 3)} ({exploitationPercent}% Policy)
                  </span>
                </div>
              ...
              <div className="pt-2 border-t border-slate-800/80 space-y-2">
                ...
                {Array.isArray(selectedNode.qValues) && selectedNode.qValues.length > 0 ? (
                  <div className="space-y-1.5">
                    {selectedNode.qValues.map((qv) => {
                      const isChosen = qv.action === selectedNode.currentAction;
                      const qNum = typeof qv.qValue === 'number' ? qv.qValue : (Number(qv.qValue) || 0);
                      const normalizedWidth = Math.max(8, Math.min(100, Math.round(((qNum + 0.5) / 1.5) * 100)));
                      ...
                      return (
                        <div key={qv.action} className="space-y-0.5">
                          <div className="flex justify-between text-[10px] font-mono">
                            ...
                            <span className={qNum >= 0 ? 'text-emerald-400 font-bold' : 'text-rose-400 font-bold'}>
                              {formatNumber(qNum, 3)}
                            </span>
                          </div>
          ...
          {/* 3. RL Economics, LCOS & Imbalance Tariff Card */}
          {!isGrid && (
            <div className="glass-card p-3 rounded-xl border border-slate-800 bg-gradient-to-br from-slate-900/90 to-slate-950/90 space-y-2">
              <div className="flex items-center justify-between text-xs text-slate-400 border-b border-slate-800 pb-1.5">
                <span className="flex items-center space-x-1.5 text-emerald-300 font-medium">
                  ...
                  <span>RL Economics & Risk Margins</span>
                </span>
                <span className="text-[9px] font-mono text-slate-400">Micro-Economics</span>
              </div>

              <div className="grid grid-cols-2 gap-2 text-[11px] font-mono">
                {/* LCOS Metric */}
                <div className="p-2 rounded-lg bg-slate-950/70 border border-slate-800 space-y-0.5">
                  <div className="text-[9px] text-slate-400 uppercase">LCOS Degradation</div>
                  <div className="font-bold text-amber-300 text-xs">
                    ${formatNumber(selectedNode.lcosPerKWh ?? 0.04, 2)}/kWh
                  </div>
                  <div className="text-[9px] text-slate-500">
                    Total: ${formatNumber(selectedNode.totalLcosCostUsd, 2)}
                  </div>
                </div>

                {/* Imbalance Penalty Tariff */}
                <div className="p-2 rounded-lg bg-slate-950/70 border border-slate-800 space-y-0.5">
                  <div className="text-[9px] text-slate-400 uppercase">Imbalance Tariff</div>
                  <div className="font-bold text-rose-400 text-xs">
                    $0.50/kWh
                  </div>
                  <div className="text-[9px] text-slate-500">
                    Penalties: ${formatNumber(selectedNode.totalImbalancePenaltiesUsd, 2)}
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* 4. Decentralized Identity & Wallet Card */}
          <div className="glass-card p-3 rounded-xl border border-slate-800 bg-gradient-to-br from-slate-900/90 to-slate-950/90 space-y-2">
            ...
            <div className="pt-1 flex items-baseline justify-between">
              ...
              <div className="text-xl font-mono font-extrabold text-slate-100">
                ${formatNumber(selectedNode.walletBalance, 2)}{' '}
                <span className="text-[10px] text-emerald-400 font-normal">USD</span>
              </div>
            </div>
          </div>

          {/* 5. Physical & Storage Telemetry */}
          {!isGrid ? (
            <div className="space-y-2.5">
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="glass-card p-2.5 rounded-xl border border-slate-800">
                  ...
                  <div className="mt-1 font-mono font-bold text-amber-300 text-base">
                    {formatNumber(selectedNode.solarGeneration ?? selectedNode.currentGenKW, 2)}{' '}
                ...
                <div className="glass-card p-2.5 rounded-xl border border-slate-800">
                  ...
                  <div className="mt-1 font-mono font-bold text-slate-100 text-base">
                    {formatNumber(selectedNode.loadConsumption ?? selectedNode.currentLoadKW, 2)}{' '}
                    <span className="text-xs text-slate-400">kW</span>
                  </div>
                  {Number(selectedNode.deferrableLoadKWh) > 0 && (
              ...
              <div className="glass-card p-3 rounded-xl border border-slate-800 bg-gradient-to-br from-slate-900/90 to-slate-950/90 space-y-2.5">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="text-[10px] font-mono text-slate-400">
                    {selectedNode.latitude !== undefined && selectedNode.latitude !== null
                      ? `${formatNumber(selectedNode.latitude, 2)}°N, ${formatNumber(selectedNode.longitude, 2)}°E`
                      : 'Geo-Tagged'}
                  </span>
                </div>
                ...
                <div className="pt-1 space-y-1.5">
                  ...
                  {selectedNode.weather?.hourlyRadiation && Array.isArray(selectedNode.weather.hourlyRadiation) && selectedNode.weather.hourlyRadiation.length > 0 && (
                    <div className="grid grid-cols-3 gap-1.5 pt-0.5">
                      {selectedNode.weather.hourlyRadiation.slice(0, 3).map((rad, idx) => (
                        <div key={idx} className="p-1.5 rounded-lg bg-slate-950 border border-slate-800 text-center font-mono">
                          ...
                          <div className="text-[10px] font-bold text-amber-300">{Math.round(Number(rad || 0))} W</div>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="glass-card p-3 rounded-xl border border-slate-800 space-y-1.5">
                  <div className="flex justify-between items-center text-xs">
                    ...
                    <span className="font-mono font-bold text-slate-200">
                      {batteryPercent}% ({currBatt}/{maxBatt} kWh)
                    </span>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="glass-card p-2.5 rounded-xl border border-cyan-900/50">
                  ...
                  <div className="mt-1 font-mono font-bold text-cyan-300 text-base">
                    {formatNumber(selectedNode.stats?.totalDispatchedKwh, 1)} kWh
                  </div>
                </div>
                <div className="glass-card p-2.5 rounded-xl border border-cyan-900/50">
                  ...
                  <div className="mt-1 font-mono font-bold text-emerald-300 text-base">
                    {formatNumber(selectedNode.stats?.carbonOffsetKg, 1)} kg
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* 6. Decommission Node (Delete) Action */}
          {!isGrid && (
          ...
          {/* 7. Signed Cryptographic Trade History */}
          <div className="space-y-2">
            ...
            {selectedNode.history && Array.isArray(selectedNode.history) && selectedNode.history.length > 0 ? (
              <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                {selectedNode.history.map((tx) => {
                  const isPenalty = tx.type === 'IMBALANCE_PENALTY';
                  const isBuyer = tx.buyerId === selectedNode.id;
                  const totalCostStr = formatNumber(tx.totalCost, 2);

                  if (isPenalty) {
                    return (
                      <div
                        key={tx.id || Math.random()}
                        className="p-2 rounded-lg bg-rose-950/30 border border-rose-800/60 text-[11px] flex items-center justify-between"
                      >
                        <div className="flex items-center space-x-2">
                          <AlertTriangle className="w-3.5 h-3.5 text-rose-400 shrink-0" />
                          <div>
                            <div className="font-bold text-rose-300">
                              Imbalance Shortfall Tariff
                            </div>
                            <div className="text-[9px] text-rose-400/80 font-mono">
                              {tx.timestamp || 'Just now'} • Utility Covered
                            </div>
                          </div>
                        </div>
                        <div className="text-right font-mono">
                          ...
                          <div className="text-[10px] text-slate-400">{tx.amountKwh} kWh @ $0.50</div>
                        </div>
                      </div>
                    );
                  }
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          <div className="font-medium text-slate-200">
                            {isBuyer ? `Bought from ${tx.sellerName || 'Seller'}` : `Sold to ${tx.buyerName || 'Buyer'}`}
                          </div>
                          <div className="text-[9px] text-cyan-400/80 font-mono">
                            {tx.txHash ? `${tx.txHash.slice(0, 10)}...` : tx.timestamp || 'Settled'} • {tx.type === 'P2P' ? 'DA Cleared' : tx.type}
                          </div>
                        </div>
                      </div>
                      <div className="text-right font-mono">
                        <div className={isBuyer ? 'text-orange-300 font-bold' : 'text-emerald-300 font-bold'}>
                          {isBuyer ? `-$${totalCostStr}` : `+$${totalCostStr}`}
                        </div>
