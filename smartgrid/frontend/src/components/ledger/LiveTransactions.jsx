      ...
      <div className="px-4 py-2.5 flex items-center justify-between bg-slate-900/80">
        <div className="flex items-center space-x-3">
          ...
          <span className="text-xs text-slate-400 font-mono hidden sm:inline">
            Total Blocks: {transactions.length} | Real-Time P2P Smart Contracts
          </span>
        </div>

        {/* Live Mini Ticker (latest transaction preview) */}
        {transactions.length > 0 && (
          <div className="hidden md:flex items-center space-x-3 text-xs font-mono bg-slate-950/60 px-3 py-1 rounded-lg border border-slate-800/80 max-w-lg truncate">
          ...
          <table className="w-full text-left text-xs font-mono">
            <thead>
              <tr className="text-slate-400 border-b border-slate-800 pb-2">
                ...
                <th className="pb-2 font-medium text-right">TOTAL SETTLED</th>
            ...
            <tbody className="divide-y divide-slate-800/40">
              {transactions.map((tx, idx) => {
                ...
                const isFeedIn = tx.type === 'GRID_FEEDIN';
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-2">
                      ...
                      >
                        {isP2P ? 'P2P Solar Trade' : isImport ? 'Grid Import' : 'Feed-In'}
                      </span>
                    </td>
                    <td className="py-2 text-emerald-400 font-medium">{tx.sellerName}</td>
                    <td className="py-2 text-orange-400 font-medium">{tx.buyerName}</td>
                    <td className="py-2 text-right font-bold text-slate-200">{tx.amountKwh} kWh</td>
                    <td className="py-2 text-right text-slate-400">${tx.pricePerKwh.toFixed(2)}</td>
