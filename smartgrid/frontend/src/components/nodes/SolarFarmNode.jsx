...
import { Sun, BatteryCharging, Zap, Wallet, Factory } from 'lucide-react';
import { useGridStore } from '../../store/gridStore';

const SolarFarmNode = ({ id, data, selected }) => {
  ...
  return (
    <div
      ...
      className={`relative w-64 rounded-xl transition-all duration-300 cursor-pointer overflow-hidden backdrop-blur-md ${
        isCurrentlySelected
          ? 'ring-2 ring-amber-400 shadow-[0_0_25px_rgba(245,158,11,0.45)] bg-slate-900/90'
          : isSelling
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-slate-800/80 bg-slate-800/30">
        <div className="flex items-center space-x-2">
          ...
          <div>
            ...
            <span className="text-[10px] text-amber-400 font-mono">Solar Array Agent</span>
          </div>
        </div>
        ...
        >
          <span className={`w-1.5 h-1.5 rounded-full ${isSelling ? 'bg-amber-400' : 'bg-slate-400'}`}></span>
          <span>{data.tradingStatus || 'Generating'}</span>
        </div>
      </div>

      {/* Body Content */}
      <div className="p-3 space-y-2.5">
        {/* Solar Gen vs Parasitic Load */}
        ...
        <div className="space-y-1">
          <div className="flex justify-between items-center text-[10px]">
            ...
            <span className="font-mono text-amber-300 font-bold">{batteryPercent}% ({data.battery}/{data.maxBattery} kWh)</span>
          </div>
          <div className="w-full h-1.5 bg-slate-800 rounded-full overflow-hidden border border-slate-700/50">
            <div
              className="h-full transition-all duration-500 rounded-full bg-gradient-to-r from-amber-500 to-yellow-400"
...
export default memo(SolarFarmNode);
